...
public:
    ...
    template<typename OtherElementType> requires (std::is_convertible_v<OtherElementType*, ElementType*>)
    StridedView(const StridedView<OtherElementType, SizeType>& other) : m_address(nullptr), m_stride(other.Stride()), m_num(other.num()) {
        if (other.num() > 0) {
            m_address = &other[0];
        }
    }
...
private:
    ...
    ElementType* GetElementPtrUnsafe(SizeType index) const {
        ...
        ElementType* asElement = reinterpret_cast<ElementType*>(asBytes + static_cast<std::size_t>(index) * static_cast<std::size_t>(m_stride));
        return asElement;
    }
...
template<typename ContainerType>
auto MakeConstStridedView(const ContainerType& container) -> ConstStridedView<std::remove_reference_t<decltype(container[0])>> {
