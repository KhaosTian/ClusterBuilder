...
public:
    ...
    StridedView(SizeType bytesBetweenElements, OtherElementType* firstElementPtr, SizeType numElements):
        m_FirstElementPtr(firstElementPtr),
        m_BytesBetweenElements(bytesBetweenElements),
    ...
    template<typename OtherElementType, typename = std::enable_if_t<std::is_convertible<OtherElementType*, ElementType*>::value>>
    StridedView(const StridedView<OtherElementType, SizeType>& other): m_FirstElementPtr(nullptr), m_BytesBetweenElements(other.GetStride()), m_NumElements(other.Num()) {
        if (other.Num() > 0) {
...
private:
    ...
    ElementType* GetElementPtrUnsafe(SizeType index) const {
        ...
        ElementType* asElement = reinterpret_cast<ElementType*>(asBytes + static_cast<std::size_t>(index) * static_cast<std::size_t>(m_BytesBetweenElements));
        return asElement;
    }
