...
HashTable::HashTable(uint32_t hashSize, uint32_t indexSize): m_hash_size(hashSize), m_hash_mask(0), m_index_size(indexSize), m_hash(EmptyHash), m_next_index(nullptr) {
    // 确保哈希表的大小大于0且是2的幂次方
    CHECK(m_hash_size > 0);
    ...
    if (m_index_size) {
        ...
        // 分配哈希桶的头索引和链表
        m_hash      = new uint32_t[m_index_size];
        m_next_index = new uint32_t[m_index_size];
        // 初始化数组元素为0xff
...
{
    if (m_index_size) {
        m_hash      = new uint32_t[m_hash_size];
        m_next_index = new uint32_t[m_index_size];

        // 拷贝内存
        std::memcmp(m_hash, other.m_hash, m_hash_size * sizeof(uint32_t));
        std::memcmp(m_next_index, other.m_next_index, m_index_size * sizeof(uint32_t));
    }
}

HashTable::HashTable(HashTable&& other) noexcept: m_hash_size(other.m_hash_size), m_hash_mask(other.m_hash_mask), m_index_size(other.m_index_size), m_hash(other.m_hash), m_next_index(other.m_next_index) {
    other.m_hash_size  = 0;
    other.m_hash_mask  = 0;
...
void HashTable::Remove(uint32_t key, uint32_t index) {
    ...
    // 从头节点开始遍历
    for (uint32_t i = m_hash[key]; IsValid(i); i = m_next_index[i]) {
