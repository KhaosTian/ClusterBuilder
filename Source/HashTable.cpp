...
FORCEINLINE HashTable::HashTable(uint32 hash_size, uint32 index_size): m_hash_size(hash_size), m_hash_mask(0), m_index_size(index_size), m_hash(EmptyHash), m_next_index(nullptr) {
    // 确保哈希表的大小大于0且是2的幂次方
    CHECK(m_hash_size > 0);
